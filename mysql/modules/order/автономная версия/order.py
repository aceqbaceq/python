...
# initial settings
date_end = datetime.date(2023, 3, 1)
j_max=1000
#
...
# order_id
#
query = ("SELECT order_id,date FROM `order`"
...
for x in myresult2:
       ...
       #print "\n"
if l == 0:
   print "'order_id' before 'date_end=%s' is not found, please specify another date_end." % (date_end)
...
while l > 0:
     ...
     #
     query = ("SELECT order_id,date FROM `order` WHERE date <= %s ORDER BY  date ASC LIMIT 1")
     mycursor.execute(query, (date_end, ))
     ...
     for x in myresult:
         d=x[0]
         order_data=x[1]
     #d=26750607    # for hardcode setting of order_id uncomment this line
     #print "\n"
     #print "table `order`, order_id=%s | START TRANSACTION |" % (d)
     sys.stdout.write("current order_id: %d, order data = %s   \r" % (d, order_data) )
     sys.stdout.flush()
     ...
     if j == j_max:
         ...
         print "table order(data)=",order_data
