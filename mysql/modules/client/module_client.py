...
def client ( initial_client_id=None, 
            DB_Host=None, 
            DB_User=None, 
            DB_Password=None, 
            DB_Name=None, 
...
#  function 'def client'  deletes  rows in `client_id` table using 'initial_client_id'
#  'initial_client_id' specifies (client_id)  row in `client` table we want to delete
#

  if (
          initial_client_id  is None
      or  DB_Host             is None
      or  DB_User             is None
  ...
  mycursor.execute(query, (initial_client_id, ))
  ...
  #
  # step 11.5 |  driver (driver_id, related_client_id) 
  #
  A_table = "driver"
  ...
  #
  # step 19 | contact (contact_id, client_id)
  #
  A_table = "contact"
  #print "enter step with %s table"  %(A_table)
  ...
  A_table = "client"
  #print "enter step with %s table"  %(A_table)
  query = ("SELECT SQL_NO_CACHE     client_id     FROM `%s` WHERE    parent_id = %%s;")  %(A_table)
  mycursor.execute(query, (d,))
  myresult_client2 = mycursor.fetchall()
